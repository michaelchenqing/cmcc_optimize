...
public class HdfsBackupSignalFiles extends AbstractHdfsBackup {

    private static final CmccLogger logger = CmccLogger.getLogger(HdfsBackupSignalFiles.class);// 日志

    private static Marker marker = FtpFileToReidsSchedulerJob.getFtpFileProcessMarker();// 文件处理标识
    ...
    private void backupFolder(String folder, HdfsBackupModel model) {
        ...
        logger.info("====================信令获取文件耗时：" + (endFiles - startFiles) / 1000 + "s=======================");

        if (files.size() < perPackageFileNums) {
            return;
        }
        ...
        if (deletes.size() == perPackageFileNums) {// 严格按照约定数量打包
            ...
            backupFilesUpload(folder, deletes, model.getLocalTempFolder());
            long endtBackup = System.currentTimeMillis();
            logger.info("====================信令打包备份耗时：" + (endtBackup - startBackup) / 1000 + "s=======================");
        }
    }
